...
public class LendingRecordService {
    ...
    public List<LendingRecord> filterLendingRecords(LendingHistoryFilterDto filterDto) {
        ...
        if (filterDto.getBorrowerId() != null && !filterDto.getBorrowerId().isEmpty()) {
            try {
                // If it's a valid email, look up the account
                if (filterDto.getBorrowerId().contains("@")) {
                    Account borrower = accountRepository.findByEmail(filterDto.getBorrowerId())
                            .orElseThrow(() -> new IllegalArgumentException("No borrower found with email " + filterDto.getBorrowerId()));
                    borrowerId = borrower.getId();
                } else {
                    // If it's a numeric string, convert it directly
                    borrowerId = Integer.parseInt(filterDto.getBorrowerId());
                }
            } catch (NumberFormatException e) {
                throw new IllegalArgumentException("Invalid borrower identifier: " + filterDto.getBorrowerId());
            }
        }

        // Use the repository method for database-level filtering
    ...
    public Page<LendingRecord> filterLendingRecordsPaginated(LendingHistoryFilterDto filterDto, Pageable pageable) {
        ...
        // Use the repository method for database-level filtering with pagination
        return lendingRecordRepository.filterLendingRecords(
                filterDto.getFromDate(),
                filterDto.getToDate(),
                status,
     ...
     * @param userId The ID of the user making the change (can be null)
    ...
    /**
     * Updates the status of a lending record with user email instead of ID.
     *
     * @param id The ID of the record to update
     * @param newStatus The new status to set
     ...
     * @param reason The reason for the status change
     * @return ResponseEntity with the result of the operation
     * @throws IllegalArgumentException if no record or user is found
     */
    @Transactional
    public ResponseEntity<String> updateStatusByEmail(int id, LendingStatus newStatus, String userEmail, String reason) {
        // Convert email to user ID if provided
        Integer userId = null;
        if (userEmail != null && !userEmail.isEmpty()) {
            Account user = accountRepository.findByEmail(userEmail)
                    .orElseThrow(() -> new IllegalArgumentException("No user found with email " + userEmail));
            userId = user.getId();
        }

        // Call the original method with the numeric ID
        return updateStatus(id, newStatus, userId, reason);
    }
     ...
     *
     * @param id The ID of the record to close
     * @param userEmail The email of the user closing the record
     * @param reason Reason for closing the record
     * @return ResponseEntity with the result of the operation
     * @throws IllegalArgumentException if no record or user is found
     */
    @Transactional
    public ResponseEntity<String> closeLendingRecordByEmail(int id, String userEmail, String reason) {
        // Convert email to user ID if provided
        Integer userId = null;
        if (userEmail != null && !userEmail.isEmpty()) {
            Account user = accountRepository.findByEmail(userEmail)
     ...
     */
    @Transactional
    public ResponseEntity<String> closeLendingRecordWithDamageAssessmentByEmail(
            int id, boolean isDamaged, String damageNotes, int damageSeverity,
        ...
        // Call the original method with the numeric ID
        return closeLendingRecordWithDamageAssessment(id, isDamaged, damageNotes, damageSeverity, userId, reason);
    }
