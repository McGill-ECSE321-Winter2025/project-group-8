...
public class LendingRecordController {
     ...
     *
     * @param ownerId The email of the game owner
     * @return ResponseEntity containing lending records for the owner
     */
    @GetMapping("/owner/{ownerId}")
    public ResponseEntity<List<LendingRecordResponseDto>> getLendingHistoryByOwner(@PathVariable String ownerId) {
        try {
            GameOwner owner = (GameOwner) accountService.getAccountByEmail(ownerId);
     ...
     *
     * @param borrowerId The email of the borrower
     * @return ResponseEntity containing lending records for the borrower
     */
    @GetMapping("/borrower/{borrowerId}")
    public ResponseEntity<List<LendingRecordResponseDto>> getLendingRecordsByBorrower(@PathVariable String borrowerId) {
        try {
            Account borrower = accountService.getAccountByEmail(borrowerId);
    ...
    public ResponseEntity<List<LendingRecordResponseDto>> getActiveLendingRecordsByBorrower(@PathVariable String borrowerId) {
        try {
            Account borrower = accountService.getAccountByEmail(borrowerId);
    ...
    public ResponseEntity<String> markGameAsReturned(
        ...
        try {
            ...
            return lendingRecordService.updateStatusByEmail(id, LendingStatus.OVERDUE, userId,
     ...
     * @param userId The email of the user confirming the return
    ...
    public ResponseEntity<Map<String, Object>> confirmGameReturn(
        ...
        try {
            ...
            if (userId != null && !userId.isEmpty()) {
                userIdNum = accountService.getAccountByEmail(userId).getId();
            }

            // Close the lending record with damage assessment
            ResponseEntity<String> result = lendingRecordService.closeLendingRecordWithDamageAssessmentByEmail(
    ...
    public ResponseEntity<Map<String, Object>> filterLendingRecords(
        ...
        try {
            // If borrowerId (email) is provided, convert it to the account ID first
            if (filterDto.getBorrowerId() != null && !filterDto.getBorrowerId().isEmpty()) {
                try {
                    // This creates a new version of the filter with the borrower ID set from the email lookup
                    Account borrower = accountService.getAccountByEmail(filterDto.getBorrowerId());

                    // The service still expects a numeric ID, so we'll need to create a temporary
                    // filter DTO for the service with the numeric ID. In a real implementation,
                    // we might want to refactor the service to also use emails.
                    LendingHistoryFilterDto serviceFilterDto = new LendingHistoryFilterDto(
                            filterDto.getFromDate(),
                            filterDto.getToDate(),
                            filterDto.getStatus(),
                            filterDto.getGameId(),
                            String.valueOf(borrower.getId())  // For service compatibility
                    );

                    // Use this modified filter
                    List<LendingRecord> filteredRecords = lendingRecordService.filterLendingRecords(serviceFilterDto);

                    // Continue with pagination as before
                    int start = page * size;
                    ...
                    return ResponseEntity.badRequest().body(Map.of(
                            "success", false,
                            "message", "Borrower not found: " + filterDto.getBorrowerId()
                    ));
                }
            } else {
                // Original logic for when no borrower email is provided
    ...
    public ResponseEntity<?> createLendingRecord(@RequestBody Map<String, Object> requestDetails) {
        try {
            ...
            String ownerId = (String) requestDetails.get("ownerId");
    ...
    public ResponseEntity<List<LendingRecordResponseDto>> getOverdueRecordsByOwner(@PathVariable String ownerId) {
        try {
            GameOwner owner = (GameOwner) accountService.getAccountByEmail(ownerId);
            List<LendingRecord> allRecords = lendingRecordService.getLendingRecordsByOwner(owner);
    ...
    public ResponseEntity<Map<String, Object>> updateLendingRecordStatus(
        ...
        try {
            ...
            // Convert user email to ID if provided
            Integer userIdNum = null;
            if (statusDto.getUserId() != null && !statusDto.getUserId().isEmpty()) {
                try {
                    Account user = accountService.getAccountByEmail(statusDto.getUserId());
                    userIdNum = user.getId();
                } catch (IllegalArgumentException e) {
                    return ResponseEntity.badRequest().body(
                            Map.of(
                                    "success", false,
                                    "message", "User not found: " + statusDto.getUserId(),
                                    "recordId", id
                            )
                    );
                }
            ...
            ResponseEntity<String> serviceResult = lendingRecordService.updateStatusByEmail(
